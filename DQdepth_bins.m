...
% plot average track depth = 2.01 m
...
%%
subplot('position',[0.1 0.1 0.3 0.8]); hold on; view([90 90])
xlabel('Depth (m)'); xlim([0 2.8]); box on;
...
% plot average track depth = 2.01 m
plot([2.01 2.01],[0 1],'color',[0.75 0.75 0.75])
...
%% from Alex's diagram, have depths along approximate track
pixelpoints = [0 23.6 57.8 33.20 36.87 34.83 50.49 53.27 44.51 38.94 49.24 32.80 38.43 33.04];
pixeldist = cumsum(pixelpoints); 
mapdepth = [2.8 1.8 1.8 2.1 2.1 2 2 1.8 1.5 1.5 1.5 1.8 2.8 2.7]; 
figure(6); clf; hold on
set(gcf,'position',[1406         -41         578         194],'paperpositionmode','auto')
plot(pixeldist,-mapdepth,'k')
ylim([-3.5 0]); xlim([0 pixeldist(end)])
% add each point marker like Alex's 
for i = 1:length(pixeldist)
    plot([pixeldist(i) pixeldist(i)],-[mapdepth(i) mapdepth(i)-0.1],'k')
end
cd /Users/julievanderhoop/Documents/MATLAB/DQ/DQ2013/AnalysisFigs
print('C9_LagoonLapDepth','-dsvg','-r300')
