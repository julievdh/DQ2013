...
%% Lono Assume 70 F temp for the warer
    ...
    Cd = 0.008; % Coeffiecent of drag from CFD
    ...
    SA = 2.93973419; %From CFD
    AM = Ca*density*Vol; % Added mass of the water
    mBar = m + AM; %Mass of the animal and the virtual mass added together

    Params = struct('l',l,'m',m,'density',density,'Cd',Cd,'Ca',Ca,'Vol',Vol,'Kin_Visosity',Kin_Visosity,'Vave',Vave,...
                    'Re',Re,'SA',SA,'AM',AM,'mBar',mBar) 

%% Calculate the forces acting on the animal in the forward and lateral directions
    
[m n] = size(strokes.A_frwd);

    for i = 1:n
        F_Drag_frwd(:,i)   = (0.5*Cd*density*SA).*(strokes.V_frwd(:,i).^2); %Use simple model to estimate drag
        F_Inert_frwd(:,i)  = mBar.*strokes.A_frwd(:,i); %Calculate inertial forces using estimated specific acceleration and the virtual mass
        F_Thrust_frwd(:,i) = F_Inert_frwd(:,i)+F_Drag_frwd(:,i); % Estimate the  thrust generated by the animal

        F_Drag_ltrl(:,i)   = (0.5*Cd*density*SA).*(strokes.V_ltrl(:,i).^2); %Use simple model to estimate drag
        F_Inert_ltrl(:,i)  = mBar.*strokes.A_ltrl(:,i); %Calculate inertial forces using estimated specific acceleration and the virtual mass
        F_Thrust_ltrl(:,i) = F_Inert_ltrl(:,i)+F_Drag_ltrl(:,i); % Estimate the  thrust generated by the animal
    end
...
%% Plot the Powers
    ...
    Lono_A_Power = struct('Power',Power,'Work',Work,'Work_ave',Work_ave,'Force', Force, 'Force_ave',Force_ave,'Params', Params)
    
    save Lono_A_Power Lono_A_Power
